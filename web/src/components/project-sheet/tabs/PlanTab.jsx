...
function ChatIA({ respuestas, setRespuestas, onGenerarDocumento, docGenerado }) {
  ...
  return (
    <div className="bg-surface-card border border-brand-100 rounded-xl p-6">
      ...
      {/* Progreso */}
      <div className="w-full h-2 bg-surface-base rounded-full overflow-hidden mb-4">
        <div
      ...
      <div className="flex gap-3 mb-4">
        <div className="w-8 h-8 rounded-full bg-brand-700 text-white flex items-center justify-center text-xs font-semibold shrink-0">IA</div>
        ...
        <div className="mb-4 space-y-2 max-h-48 overflow-y-auto">
          {Object.entries(respuestas).map(([id, r]) => (
            <div key={id} className="flex gap-3 justify-end">
              ...
              <div className="w-8 h-8 rounded-full bg-brand-100 text-brand-800 flex items-center justify-center text-xs font-semibold shrink-0">U</div>
            </div>
          ))}
        </div>
      ) : null}
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-2">
            {indicePregunta > 0 ? (
              <button type="button" onClick={volver} className="px-3 py-2 rounded-xl border border-brand-200 text-brand-800 text-sm hover:bg-surface-base transition-colors">
                Anterior
              </button>
            ) : null}
            <button type="button" onClick={saltar} className="px-3 py-2 rounded-xl border border-brand-200 text-surface-muted text-sm hover:bg-surface-base transition-colors">
              Saltar
            </button>
          </div>
          {esUltima ? (
            ...
            <button
              ...
              onClick={responder}
              disabled={!input.trim()}
              className="px-4 py-2 rounded-xl bg-brand-700 text-white text-sm font-medium hover:bg-brand-800 disabled:opacity-50 transition-colors"
            >
              Responder
            </button>
          )}
...
function PlanGenerated({ proyecto, onVerify }) {
  ...
  const [plan, setPlan] = useState(() => ({
    ...
    presupuestoEstimado: planMaestroMock.presupuestoEstimado,
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-surface-card border border-brand-100 rounded-xl p-5">
          ...
          <p className="mt-1.5 text-sm text-brand-800">Estancias</p>
          <p className="text-xs text-surface-muted">Estancias detectadas</p>
        </div>
        <div className="bg-surface-card border border-brand-100 rounded-xl p-5">
          <p className="text-2xl font-semibold text-brand-900 leading-none">{plan.m2Totales} m²</p>
          <p className="mt-1.5 text-sm text-brand-800">m² totales</p>
        </div>
        <div className="bg-surface-card border border-brand-100 rounded-xl p-5">
          <p className="text-2xl font-semibold text-brand-900 leading-none">~{formatEur(plan.presupuestoEstimado)}</p>
          <p className="mt-1.5 text-sm text-brand-800">Presupuesto est.</p>
        </div>
      </div>
      ...
      {/* Trabajos editables (chips toggle) */}
      <div className="bg-surface-card border border-brand-100 rounded-xl p-6">
      ...
      <div className="bg-surface-card border border-brand-100 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-sm font-semibold uppercase tracking-wider text-surface-muted">Materiales sugeridos</h4>
